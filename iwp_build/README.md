...
uv run iwp-build history checkpoint --config .iwp-lint.yaml --message "fast loop savepoint" --json out/history-checkpoint.json
...
uv run iwp-build history checkpoint --config .iwp-lint.yaml --message "agent: pre-alignment savepoint"
...
uv run iwp-build history checkpoint --config .iwp-lint.yaml --message "dev checkpoint" --json out/history.checkpoint.json
...
3. `history checkpoint`: create a pure file-level checkpoint for fast iteration snapshots (no gate)
4. `session commit`: run gate and atomically advance baseline as the regular commit checkpoint writer
5. `history restore`: switch current baseline pointer to a historical checkpoint when rollback/forward-jump is needed
6. `verify`: run compiled checks, full lint gate, and optional regression tests
7. `watch` (optional local loop): incremental `.iwc` compile only; not a workflow checkpoint
...
- `session reconcile` runs `diff -> (optional normalize) -> gate` and returns:
  - `meta` (`protocol_block`, `mode`, `schema_version`, `workflow_mode`)
