...
from .diff_resolver import impacted_nodes, load_diff
from .errors import Diagnostic
from .md_parser import parse_markdown_nodes
from .models import CoverageMetrics, LinkAnnotation, MarkdownNode
from .schema_validator import validate_markdown_schema
...
def _run_core(
    ...
    if mode == "diff" and changed_code_files is not None:
        ...
        links = [
            ...
            if link_item.file_path in changed_code_files
            or link_item.source_path in (changed_md_files or set())
        ]

    diagnostics.extend(scan_diagnostics)
    diagnostics.extend(validate_link_protocol(links))

    node_index: dict[NodeKey, MarkdownNode] = {
        (node.source_path, node.node_id): node for node in target_nodes
    }
    node_source_index: set[NodeKey] = {(node.source_path, node.node_id) for node in target_nodes}
    source_paths = {node.source_path for node in target_nodes}
    ...
    for link in links:
        ...
        if link.source_path not in source_paths:
            diagnostics.append(
                Diagnostic(
                    code="IWP103",
            ...
            continue
        if (link.source_path, link.node_id) not in node_source_index:
            diagnostics.append(
                Diagnostic(
                    code="IWP105",
                    message=(
                        f"node_id does not exist in source_path: {link.source_path}::{link.node_id}"
                    ),
                    file_path=link.file_path,
                    line=link.line,
                    column=link.column,
        ...
        node = node_index[(link.source_path, link.node_id)]
